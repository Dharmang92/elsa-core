          ...
          <div class="flex flex-col flex-1">
            ...
            {orientation == 'Landscape' && (
              <div class="px-10 py-4 bg-gray-50">
                <div class="flex items-start justify-between space-x-3">
                  <div class="space-y-0">
                    <h2 class="text-md font-medium text-gray-900">{mainTitle}</h2>
            ...
            <div class={`border-b border-gray-200 ${orientation == 'Landscape' ? 'pl-10' : 'pl-4'}`}>
              <nav class="-mb-px flex justify-start space-x-5" aria-label="Tabs">
                {tabs.map((tab, tabIndex) => {
                  ...
                  return (
                    <a href="#" onClick={e => this.onTabClick(e, tab)} class={`${cssClass} py-4 px-1 text-center border-b-2 font-medium text-sm`}>
                      {tab.displayText}
                    </a>
                  );
                })}
            ...
            <div class={`flex-1 relative`}>
              <div class={`absolute inset-0 overflow-y-scroll ${orientation == 'Landscape' ? 'px-6' : ''}`}>
                {tabs.map((tab, tabIndex) => {
                  ...
                  return <div class={cssClass}>{tab.content()}</div>;
                })}
              </div>
            </div>
          </div>
          ...
          {actions.length > 0 ? (
            <div class="flex-shrink-0 px-4 border-t border-gray-200 py-5 sm:px-6">
              <div class="space-x-3 flex justify-end">
                {actions.map(action => {
                  if (action.display) return action.display(action);
